...
#include "Model.h"        // for AUDIO_DIRECTORY
...
//
iSound* CreateSound(const wchar_t* f, bool c) {

  return new Sound(f, false, c, c, 0, 0);
}

iSound* CreateLocalSound(const wchar_t* f, bool c, float q, 
  float i) {

    return new Sound(f, true, c, false, q, i);
}
