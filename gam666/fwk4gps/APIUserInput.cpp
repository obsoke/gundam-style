...
APIUserInput::APIUserInput(const wchar_t* a) : audioDirectory(a) {
    ...
    action = new unsigned[nActions][3];
...
// getWindowWidth returns the window width for full screen mode
//
int APIUserInput::getWindowWidth() const {

    return runinwndw ? 0 : displaySet->getWidth(displayId, modeId, pixelId);
}

// getWindowHeight returns the window height for full screen mode
//
int APIUserInput::getWindowHeight() const {

    return runinwndw ? 0 : displaySet->getHeight(displayId, modeId, pixelId);
}
...
// release releases the key for Action a
//
void APIUserInput::release(Action a) {
