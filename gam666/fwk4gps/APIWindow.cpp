...
#include "Translation.h"  // for WND_CAPTION

bool registerAPIWindowClass(HINSTANCE hinst);
LRESULT CALLBACK wndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp);

#define WND_FULL_NAME WND_CAPTION
...
// processMessage checks extracts a message from the message queue and
...
LRESULT CALLBACK wndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
  ...
  //coordinator has quit program from alternate means
  if (coordinator->quit() && !quit) { 
	  wp = VK_ESCAPE;
	  msg = WM_KEYDOWN;
  }
